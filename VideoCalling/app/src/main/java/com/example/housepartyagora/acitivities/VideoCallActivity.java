...
public class VideoCallActivity extends AppCompatActivity {public static final int LAYOUT_TYPE_DEFAULT = 0;
    ...
    private final IRtcEngineEventHandler mRtcEventHandler = new IRtcEngineEventHandler() {
        ...
        public void onJoinChannelSuccess(String channel, final int uid, int elapsed) {
            ...
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    showToast("User: " + getUserName() + " join!");
        ...
        public void onUserOffline(final int uid, int reason) {
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    showToast("User: " + getUserName() + " left the room.");
    ...
    private void getExtras() {
        ...
        Log.i("username", "getExtras: "+userName);
        user = new User();
    }
    ...
    private void initEngineAndJoinChannel() {
        initializeEngine();
        setupVideoConfig();
        loginRTM();//for real time messaging
        setupLocalVideo();
        joinChannel();//joining channel with your username
    }
    ...
    private void joinChannel() {
        ...
        //String token="00660b125b1a6094994ad354f35f0c2c9b5IACJWV4UY0YOpw7xYueqvqq1E2jLF6TE7+9i3efRQcpJ6MtjlTkAAAAAEAAm+nFWIhHUYAEAAQAgEdRg/Qds60OS+VguOf93mkR6U0OHqGoTGnpeAx7fg0BD+KwAAAAAEAAm+nFWgWrTYAEAAQCAatNg";
    ...
    private void onBigVideoViewDoubleClicked(View view, int position) {
        if (mUidsList.size() < 2) {
            return;
        }//if only one user ie you

        final UserStatusData user = mGridVideoViewContainer.getItem(position);
        //if double clicked person is not the user himself
    ...
    public void joinFriend(String friendName){
        channelName = friendName;//join the channel name as friend's name
        finishCalling();//finish his own call
        startCalling();//join friend's call
